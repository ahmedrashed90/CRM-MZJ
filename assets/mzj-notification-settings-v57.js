...
async function loadPushConfig(){
  const snap = await getDoc(PUSH_SETTINGS_REF);
...
async function registerPushSubscription(options={}){
  ...
  pushRegistrationPromise = (async () => {
    ...
    const subscriptionId = pushSafeId(`web_${deviceId}`);
    const subscriptionRef = doc(db, pushConfig.subscriptionCollection, subscriptionId);
    const existingSnap = await getDoc(subscriptionRef).catch(() => null);
    const existing = existingSnap?.exists?.() ? (existingSnap.data() || {}) : {};
    const now = new Date().toISOString();

    const uid = profileText(profile, ["uid", "id"]) || userDocId || user.uid;
