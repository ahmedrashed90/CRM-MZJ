...
import { getFirestore, doc, getDoc, getDocs, setDoc, collection, collectionGroup, onSnapshot, query, where, limit, serverTimestamp } from "https://www.gstatic.com/firebasejs/10.12.5/firebase-firestore.js";
...
const PUSH_SETTINGS_REF = doc(db, "settings_sources", "push_notifications");
const PUSH_SUBSCRIPTIONS_DEFAULT_COLLECTION = "push_subscriptions";
const PUSH_DEVICE_ID_KEY = "mzj_push_device_id_v1";
...
let pushRegistrationRetryArmed = false;
let pushRegistrationStatus = { ok:false, state:"idle", reason:"not_started" };
let pushMessagingModulePromise = null;

function loadPushMessagingModule(){
  if(!pushMessagingModulePromise){
    pushMessagingModulePromise = import("https://www.gstatic.com/firebasejs/10.12.5/firebase-messaging.js");
  }
  return pushMessagingModulePromise;
}
...
function getPushFirestoreApi(){
  const shared = getSharedFirestore();
  if(shared){
    return {
      makeDoc: (...segments) => shared.doc(shared.db, ...segments),
      getDoc: shared.getDoc,
      setDoc: shared.setDoc,
      serverTimestamp: shared.serverTimestamp
    };
  }
  return {
    makeDoc: (...segments) => doc(db, ...segments),
    getDoc,
    setDoc,
    serverTimestamp
  };
...
function getPushDeviceId(){
  try{
    let id = String(localStorage.getItem(PUSH_DEVICE_ID_KEY) || "").trim();
    if(!id){
      id = (globalThis.crypto?.randomUUID?.() || `${Date.now()}_${Math.random().toString(36).slice(2)}`).replace(/[^0-9A-Za-z_-]/g, "_");
...
async function loadPushConfig(){
  const fsApi = getPushFirestoreApi();
  const snap = await fsApi.getDoc(fsApi.makeDoc("settings_sources", "push_notifications"));
  const data = snap.exists() ? (snap.data() || {}) : {};
  const nested = data.pushNotifications && typeof data.pushNotifications === "object" ? data.pushNotifications : {};
  const config = { ...data, ...nested };
  return {
    enabled: config.enabled !== false,
    appName: normalize(config.appName || "MZJ CRM - Sales"),
    vapidKey: normalize(config.vapidKey || config.vapid_key || ""),
    subscriptionCollection: normalize(config.subscriptionCollection || config.subscription_collection || PUSH_SUBSCRIPTIONS_DEFAULT_COLLECTION) || PUSH_SUBSCRIPTIONS_DEFAULT_COLLECTION
  };
}
async function loadPushUserProfile(user){
  let profile = {};
  let userDocId = "";
  const fsApi = getPushFirestoreApi();

  try{
    const uidSnap = await fsApi.getDoc(fsApi.makeDoc("users", user.uid));
    if(uidSnap.exists()){
      profile = uidSnap.data() || {};
      userDocId = uidSnap.id;
    }
  }catch{}

  if(!userDocId && user.email){
    try{
      const emailSnap = await getDocs(query(collection(db, "users"), where("email", "==", user.email), limit(1)));
      if(!emailSnap.empty){
  ...
  return { profile, userDocId:userDocId || user.uid };
}
function profileText(profile, keys){
  for(const key of keys){
    const value = normalize(profile?.[key]);
    if(value) return value;
  }
  return "";
}
function armPushRegistrationRetry(){
  if(pushRegistrationRetryArmed || typeof window === "undefined") return;
  pushRegistrationRetryArmed = true;
  const retry = () => {
    pushRegistrationRetryArmed = false;
    window.removeEventListener("pointerdown", retry, true);
    window.removeEventListener("keydown", retry, true);
    registerPushSubscription({ requestPermission:true, reason:"user_interaction_retry" }).catch(() => {});
  };
...
async function registerPushSubscription(options={}){
  ...
  pushRegistrationPromise = (async () => {
    const user = options.user || await waitForNotificationAuth(12000);
    if(!user){
      pushRegistrationStatus = { ok:false, state:"skipped", reason:"no_authenticated_user" };
      return pushRegistrationStatus;
    }

    if(!globalThis.isSecureContext || !("serviceWorker" in navigator) || !("Notification" in globalThis)){
      pushRegistrationStatus = { ok:false, state:"unsupported", reason:"push_not_supported" };
      return pushRegistrationStatus;
    }

    const messagingModule = await loadPushMessagingModule();
    const supported = await messagingModule.isSupported().catch(() => false);
    if(!supported){
      pushRegistrationStatus = { ok:false, state:"unsupported", reason:"firebase_messaging_not_supported" };
      return pushRegistrationStatus;
    }

    const pushConfig = await loadPushConfig();
    if(!pushConfig.enabled){
      pushRegistrationStatus = { ok:false, state:"disabled", reason:"push_disabled_in_settings" };
      return pushRegistrationStatus;
    }
    if(!pushConfig.vapidKey){
      pushRegistrationStatus = { ok:false, state:"missing_config", reason:"missing_vapid_key" };
      return pushRegistrationStatus;
    }
    ...
    if(ios && !standalone){
      pushRegistrationStatus = { ok:false, state:"installation_required", reason:"ios_requires_home_screen_pwa" };
      return pushRegistrationStatus;
    }

    if(permission === "default" && options.requestPermission !== false){
      const isAutomaticBootAttempt = options.reason === "auth_ready";
      if(ios && isAutomaticBootAttempt){
        pushRegistrationStatus = { ok:false, state:"permission_required", reason:"ios_permission_needs_user_tap" };
        armPushRegistrationRetry();
        return pushRegistrationStatus;
      }
    ...
    if(permission !== "granted"){
      pushRegistrationStatus = { ok:false, state:"permission_required", reason:`notification_permission_${permission}` };
      if(permission === "default") armPushRegistrationRetry();
      return pushRegistrationStatus;
    }
    ...
    if(!token){
      pushRegistrationStatus = { ok:false, state:"failed", reason:"empty_fcm_token" };
      return pushRegistrationStatus;
    }

    const { profile, userDocId } = await loadPushUserProfile(user);
    const deviceId = getPushDeviceId();
    const subscriptionId = pushSafeId(`web_${user.uid}_${deviceId}`);
    const fsApi = getPushFirestoreApi();
    const subscriptionRef = fsApi.makeDoc(pushConfig.subscriptionCollection, subscriptionId);
    const existingSnap = await fsApi.getDoc(subscriptionRef).catch(() => null);
    const existing = existingSnap?.exists?.() ? (existingSnap.data() || {}) : {};
    const now = new Date().toISOString();

    const uid = user.uid;
    const profileUid = profileText(profile, ["uid", "id"]) || userDocId || user.uid;
    const email = profileText(profile, ["email"]) || normalize(user.email);
    const name = profileText(profile, ["name", "fullName", "displayName"]) || normalize(user.displayName) || email || uid;
    const department = profileText(profile, ["department", "departmentKey", "departmentName", "section", "sectionKey", "sectionLabel", "role"]);
    const departmentName = profileText(profile, ["departmentName", "departmentLabel", "sectionLabel"]) || department;
    const branch = profileText(profile, ["branch", "branchName", "branchLabel", "branchId", "branchKey"]);
    const branchName = profileText(profile, ["branchName", "branchLabel", "branch"]) || branch;

    await fsApi.setDoc(subscriptionRef, {
      id: subscriptionId,
      uid,
      userUid: uid,
      authUid: user.uid,
    ...
    pushRegistrationStatus = {
      ...
      subscriptionId,
      collection: pushConfig.subscriptionCollection,
      uid
    };

    try{
      window.dispatchEvent(new CustomEvent("mzj:push-subscription-ready", { detail:{ ...pushRegistrationStatus } }));
    }catch{}

    return pushRegistrationStatus;
  })().catch(error => {
    console.warn("MZJ push subscription registration failed", error);
    pushRegistrationStatus = { ok:false, state:"failed", reason:error?.code || error?.message || String(error) };
    armPushRegistrationRetry();
    return pushRegistrationStatus;
  }).finally(() => {
...
function runPushRegistrationWhenIdle(user){
  const run = () => {
    registerPushSubscription({ user, requestPermission:true, reason:"auth_ready" }).catch(() => {});
  };
...
function startPushRegistration(user){
  if(!user) return;
  ...
  const schedule = () => runPushRegistrationWhenIdle(user);
...
window.MZJPushRegistration = {
  register: () => registerPushSubscription({ requestPermission:true, reason:"manual" }),
  getStatus: () => ({ ...pushRegistrationStatus })
};
